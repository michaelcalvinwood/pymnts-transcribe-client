...
function App() {
  ...
  const [hideDragDrop, setHideDragDrop] = useState(false);
  ...
  if (!window.socketConnection) {
    ...
    window.socketConnection.on('rawTranscript', rt => {
      setTranscript(rt);
      setHideDragDrop(true);
    });
  ...
  return (
    <div className="App">
      ...
      {!hideDragDrop && <Progress value={progress} max={100} style={{width: '100%', marginTop: '.5rem'}} />}

      {!hideDragDrop && <Box marginTop="8px" height="96px" padding='.5rem' border='1px solid var(--chakra-colors-gray-200)' borderRadius='8px' cursor='pointer'>
          <div {...getRootProps()}>
              <input {...getInputProps()} />
              {
